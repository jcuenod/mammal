...
export type MPTreeNodeWithChildren = {
  node: MPTreeNode;
  children: MPTreeNodeWithChildren[];
};

export default class MPTreeNode {
  path: string;
  data: any;
  tableName: string;
  ...
  isChildOf(node: MPTreeNode) {
    return this.path.startsWith(`${node.path}.`);
  }

  isDescendantOf(node: MPTreeNode) {
    return this.path.startsWith(`${node.path}.`);
  }

  isSiblingOf(node: MPTreeNode) {
